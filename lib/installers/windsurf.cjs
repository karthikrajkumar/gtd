/**
 * GTD Installer Adapter — Windsurf runtime.
 *
 * Framework files installed under ~/.codeium/windsurf/get-things-done/.
 *
 * @module lib/installers/windsurf
 */
...
function getInstallPath(location) {
  if (location === 'global') {
    return path.join(os.homedir(), '.codeium', 'windsurf', 'get-things-done');
  }
  return path.join(process.cwd(), '.windsurf', 'get-things-done');
}

/**
 * Install commands as a workflow manifest for Windsurf.
 * Windsurf reads the manifest for available GTD workflow commands.
 */
...
function setupHooks(installBase) {
  // Windsurf does not require additional hook setup.
  return { hooks: 0 };
}
