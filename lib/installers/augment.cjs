/**
 * GTD Installer Adapter — Augment runtime.
 *
 * Framework files installed under ~/.augment/get-things-done/.
 *
 * @module lib/installers/augment
 */
...
function getInstallPath(location) {
  if (location === 'global') {
    return path.join(os.homedir(), '.augment', 'get-things-done');
  }
  return path.join(process.cwd(), '.augment', 'get-things-done');
}

/**
 * Install commands as a workflow manifest for Augment.
 * Augment reads the manifest for available GTD workflow commands.
 */
...
function setupHooks(installBase) {
  // Augment does not require additional hook setup.
  return { hooks: 0 };
}
